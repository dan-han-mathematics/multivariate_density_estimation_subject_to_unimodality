...
compute_F <- function(tdata1, tdata2, i, m) {
  k_values <- 0:m
  
  # Define a function to compute the product of pbeta values for given k1 and k2
  pbeta_product <- function(k1, k2, t1, t2) {
    pbeta(t1, shape1 = k1 + 1, shape2 = m - k1 + 1) * 
      pbeta(t2, shape1 = k2 + 1, shape2 = m - k2 + 1)
  }
  
  # Precompute pbeta values using outer and Vectorize
  t1 <- tdata1[i]
  t2 <- tdata2[i]
  pbeta_values <- outer(k_values, k_values, Vectorize(function(k1, k2) pbeta_product(k1, k2, t1, t2)))
  
  # Convert the outer product matrix to a vector
  F <- as.vector(t(pbeta_values))
  
  return(F)
}

# Main function to compute G matrix
compute_G <- function(tdata1, tdata2, n, m) {
  # Pre-allocate the G matrix
  G <- matrix(0, nrow = (m + 1)^2, ncol = n)
  
  # Fill in the G matrix column by column
  for (i in 1:n) {
    G[, i] <- compute_F(tdata1, tdata2, i, m)
  }
...
umd_multi <- function(data,crit="CN", m=NA, warning=TRUE){
  ...
  if( crit == "CN"){
    ...
    if( is.na(m) == FALSE){
      ...
      G1 <- compute_G(tdata1, tdata2, n, m)
      G=t(G1)
      Amat <- t(G) %*% D %*% G				
      bvec <- 2*t(g)%*% D %*% G
    ...
    dme_un=function(x){
      ...
      mix.pdf=function(x){
        ...
        x1_scaled <- (x[1] - sort.data1[1]) / (sort.data1[n] - sort.data1[1])
        x2_scaled <- (x[2] - sort.data2[1]) / (sort.data2[n] - sort.data2[1])
        
        beta1 <- sapply(0:m, function(k1) dbeta(x1_scaled, shape1 = k1 + 1, shape2 = m - k1 + 1))
        beta2 <- sapply(0:m, function(k2) dbeta(x2_scaled, shape1 = k2 + 1, shape2 = m - k2 + 1))
        
        B_m <- as.vector(t(outer(beta1, beta2)))
    ...
    return(list(weights=weights,m.hat=m,dme_un=dme_un,running_time=running_time))
  }}
