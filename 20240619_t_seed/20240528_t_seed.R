...
setwd("C:/Users/Dan Han/OneDrive - University of Louisville/Research Shared with Collaborator/Multivariate Density Estimation Subject to Unimodality/20240528_t_seed")
...
RMISE_ks<- function (K){
  ...
  for(k in 1:K){
    ...
    d=(max(data[,1])-min(data[,1]))*(max(data[,2])-min(data[,2]))/(n_dim^2)
    Hpi1 <- Hpi(x=data)
    ks_result= ks::kde(x=data, H=Hpi1,eval.points = data, bgridsize=c(20,20))
...
RMISE_locpol<- function (K){
  ...
  for(k in 1:K){
    ...
    n_dim=nrow(data)
    d=(max(data[,1])-min(data[,1]))*(max(data[,2])-min(data[,2]))/(n_dim^2)
    dDen <- bivDens(as.data.frame(data),weig=1/data[,1])
    density_estimate<-predict(dDen,newdata=as.data.frame(data))
    true_density=dmst(data,xi=rep(0,length(alpha)), Omega=Sigma, alpha=alpha,nu=6)
    diff_locpol=density_estimate-true_density
    sum_e2=0
    for(i in 1:n_dim){
      sum_e2=sum_e2+(abs(diff_locpol[i]))^2
    }
    RMISE_SUM=RMISE_SUM+sqrt(d*sum_e2)
  }
  RMISE_estimate=RMISE_SUM/K
...
library(ggplot2)
boxplot_rmise_t=ggplot(rmise_t, aes(x=group,y=RMISE)) + 
  geom_boxplot()+
  coord_cartesian(ylim = c(0, max(c(rmise3_umd,rmise3_kde2d,rmise3_ks,rmise3_kdevine))))

png(file = "boxplot_rmise_t.png", bg = "transparent",width=3000, height=2000, res=800, units="px")
boxplot_rmise_t
dev.off()
...
########################################################
#The following generates ML1E for t Distribution#	
###########################################################
ML1E_umd<- function (K){
  ML1E_SUM=0
  for(k in 1:K){
...
ML1E_locpol<- function (K){
  ...
  for(k in 1:K){
    ...
    sum_abe=0
    for(i in 1:n_dim){
      sum_abe=sum_abe+abs(diff_locpol[i])
    }
    ML1E_SUM=ML1E_SUM+d*sum_abe
  }
  ML1E_estimate=ML1E_SUM/K
...
library(ggplot2)
boxplot_ML1E_t=ggplot(ML1E_t, aes(x=group,y=ML1E)) + 
  geom_boxplot()+
  coord_cartesian(ylim = c(0, max(ML1E3_kde2d,ML1E3_umd,ML1E3_ks,ML1E3_kdevine)))

png(file = "boxplot_ML1E_t.png", bg = "transparent",width=3000, height=2000, res=800, units="px")
boxplot_ML1E_t
dev.off()
...
########################################################
#The following generates ML_infE for t Distribution#	
###########################################################
...
library(ggplot2)
boxplot_ML_infE_t=ggplot(ML_infE_t, aes(x=group,y=ML_infE)) + 
  geom_boxplot()+
  coord_cartesian(ylim = c(0, max(ML_infE3_kde2d, ML_infE3_umd,ML_infE3_ks,ML_infE3_kdevine)))

png(file = "boxplot_ML_inf_t.png", bg = "transparent",width=3000, height=2000, res=800, units="px")
boxplot_ML_infE_t
dev.off()
...
save.image("20240528_t_seed.RData")
